import {concat, range, forEach, zipWith, split, map, toNumber, findIndex, last} from 'lodash'
...
export default class ProppedBeam {
    ...
    RL(){
        ...
        forEach(this.PL(), (Pi,j)=>{
            ...
            let a = Pi.a
            let b = L - a
            Ri =  Ri + P*Math.pow(b,2)/(2*Math.pow(L,3))*(a+2*L)
        }) 
        return Ri
    }//RR
    ...
    RR(){
        ...
        forEach(this.PL(), (Pi,j)=>{
            ...
            let a = Pi.a
            let b = L - a
            Ri =  Ri + P*a/(2*Math.pow(L,3))*(3*Math.pow(L,2)-Math.pow(a,2))
        }) 
        return Ri
    }//RR
    ...
    Mx(){
        ...
        forEach(this.Lx(), (x,i) =>{
            ...
            forEach(this.PL(), (Pi,j)=>{
                ...
                if(x <= a){
                    Mi = Mi + P*Math.pow(b,2)/(2*Math.pow(L,3))*(a+2*L)*x
                }//IF
                else{
                    Mi = Mi + P*Math.pow(b,2)/(2*Math.pow(L,3))*(a+2*L)*x - P*(x-a)
                }//ELSE
            })//P LOOP
            //M.push(Mi.toFixed(4))
            M.push(Mi)
    ...
    Dx(){
        ...
        forEach(this.Lx(), (x,i) =>{
            Di = w*x*1728*(Math.pow(L,3)-3*L*Math.pow(x,2)+2*Math.pow(x,3))/(24*E*I)
